...
//			$(window).bind("pageshow", function(event){
			$(document).ready(function(){ 
				if("PC" != "PC") {
					...
					var html = '';
					html += '<button class="mo-header-btnType01" onclick="petInsert();" data-content="954216" data-url="/my/pet/petInsertView">';
			...
			function myPetView(petNo) {
				//location.href = "/my/pet/myPetView?petNo=" + petNo;
		...
		<div class="myCate">
			<ul class="menu">
				<li class="open">
					...
					<ul class="sm">
						<li class="active"><a class="bt" href="myPetList.do" data-url="myPetList.do" data-content="954216"><b class="t">마이펫 관리</b></a></li>
...
<main class="container lnb page my" id="container">
			...
			<div class="inr" style="min-height: 577px;">
				<!-- 본문 -->
				<div class="contents" id="contents" style="min-height: 499px;">
					<div class="mypet-admin t2">
						<!-- PC 타이틀 모바일에서 제거 -->
						<div class="pc-tit">
								<h2>마이펫 관리</h2>
							<div class="right-item">
								<button class="btn sm new a" onclick="petInsert();" data-content="954216" data-url="/my/pet/petInsertView">마이펫 등록하기</button>
							</div>
						</div>
						<!-- // PC 타이틀 모바일에서 제거 -->
							...
							<section class="no_data i6 ">
								<div class="inr">
									...
									<div class="uimoreview">
										<a href="#" onclick="petInsert(); return false;" class="bt more" data-content="954275" data-url="/my/pet/petInsertView">마이펫 등록하기<span class="arrow"></span></a>
									</div>
								</div>								                               
							</section>
							<!-- // 위치추천을 위해 서비스를 활성화해주세요 -->
						...
						<div class="mypet-list">
							...
							<c:forEach items="${ petlist }" var="petDTO">
								<li>
										<p class="img">
											<img src="${ petDTO.pet_img }" onclick="fncGoPetView(${ petDTO.pet_code });" style="cursor: pointer;height: 100%;width: 100%;">
										</p>
										<div class="box">
											<p class="t1" onclick="fncGoPetView(${ petDTO.pet_code });" style="cursor: pointer;">${ petDTO.pet_name }</p>
											<p class="t2">
												<c:if test="${ petDTO.pt_name ne '-' }">
													<span>${ petDTO.pt_name }</span>
												</c:if>
												<span>${ petDTO.pet_gen }</span><span>${ petDTO.pet_age }</span>
											</p>
											<a href="#" onclick="myPetIncl(${ petDTO.pet_code })" class="btn sm" data-content="" data-url="myPetIncl(${ petDTO.pet_code } , ${ petDTO.pet_name })">건강수첩</a>
										</div>
										<a href="#" onclick="fncGoPetView(${ petDTO.pet_code });return false;" class="link" data-content="" data-url="myPetView.do?petNo=${ petDTO.pet_code }">상세정보 이동</a>
									</li>
								</c:forEach>
								</ul>
						</div>
